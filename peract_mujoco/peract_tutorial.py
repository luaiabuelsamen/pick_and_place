...
BATCH_SIZE = 1
NUM_DEMOS = 4 # total number of training demonstrations to use while training PerAct
...
def print_gpu_mem():
    ...
    print(f"[GPU Memory] Allocated: {allocated:.2f} MB | Reserved (Cached): {reserved:.2f} MB")

start_time = time.time()

for iteration in range(TRAINING_ITERATIONS):
    try:
        # Load batch and move to device safely
        batch = next(train_data_iter)
        ...
        print(f"[ERROR] Iteration {iteration} failed: {str(e)}")
        torch.cuda.empty_cache()
        gc.collect()
...
print(f"Translation (discrete): {vis_trans_coord}")
print(f"Translation (continuous): {continuous_trans}")

# Rotation (orientation in 3D space)
discrete_rot = update_dict['pred_action']['rot_and_grip'][0][:3].detach().cpu().numpy()
print(f"Rotation (discrete euler angles): {discrete_rot}")
...
# torch.save(peract_agent.model, 'peract_full_model.pth')

# OR save just the state dict (recommended, more portable)
# torch.save(peract_agent.model.state_dict(), 'peract_model_weights.pth')
# Save the key components of the agent
save_dict = {
    'perceiver_encoder': peract_agent._perceiver_encoder.state_dict(),
    'q': peract_agent._q.state_dict(),
    'optimizer': peract_agent._optimizer.state_dict(),
    'voxel_size': peract_agent._voxel_size,
    'voxel_feature_size': peract_agent._voxel_feature_size,
    'num_rotation_classes': peract_agent._num_rotation_classes,
    'rotation_resolution': peract_agent._rotation_resolution,
    'coordinate_bounds': peract_agent._coordinate_bounds
}

torch.save(save_dict, 'peract_model_components.pth')
# %%
