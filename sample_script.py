...
def save_image_thread(running_event):
    while running_event.is_set() or not image_queue.empty():
        if image_queue.empty():
            continue
        run_number, img, ego, depth_img, pos, tick = image_queue.get(timeout=0.1)
        ...
        img_path = os.path.join(run_folder, f"depth_{tick}.png")
        plt.imsave(img_path, depth_img)
        log[run_number][tick]['depth'] = img_path
    ...
    while True:
        ...
        while tick < q_traj_combined.shape[0]:
            ...
            if tick % 1 == 0:
                ...
                image_queue.put((run_number, img, rgb_img, depth_img, pos, tick))
