...
def save_image_thread():
    ...
    while True:
        if not image_queue.empty():
            img, tick = image_queue.get(timeout=0.1)
...
run_number = 1
image_thread = threading.Thread(target=save_image_thread, daemon=True)
image_thread.start()
...
while True:
    ...
    while tick < q_traj_combined.shape[0]:
        if tick % 10 == 0:
            ...
            # image_queue.put((img, tick))
            image_queue.put((rgb_img, f'ego{tick}'))
